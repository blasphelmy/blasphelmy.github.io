...
function() {
    var a, b, c, d, e, f = function() {
            ...
            for (var b in a) {
                var c = /^(http(s)?:){0,1}\/\/([a-z]{1,}\.)*?(imgur(-dev)?.com)/.exec(a[b].src);
                if (c) return c[0].indexOf("//s.imgur.com") === -1 && c[3] && c[5] ? c[0] : c[1] ? c[1] + "//imgur.com" : window.location.protocol + "//imgur.com"
            }
        },
        g = window,
        h = document,
    ...
    var r = function(a) {
        ...
        s = function(a, b) {
            if (a = parseFloat(a), isNaN(a) && (a = k), /^(a\-)?[a-z0-9]{5,7}$/gi.test(b)) {
                var d = "#imgur-embed-iframe-pub-" + b + " { height: " + a + "px !important;width:" + c + "px !important;}";
                r(d)
            }
        },
        t = function() {
            var a = ".imgur-embed-iframe-pub { box-shadow: 0px 0px 5px 0px rgba(0, 0, 0, 0.10); border: 1px solid #ddd; border-radius: 2px;} blockquote.imgur-embed-pub { width: " + m + "px; }";
            r(a)
        },
        u = function(a, b, d, e) {
        ...
        v = function(f) {
            if (f) {
                ...
                var g, i, j, l;
                try {
                    b = f.getAttribute("data-context") || "true", analytics = f.getAttribute("data-analytics") || "true", g = f.getAttribute("data-id"), i = 0 === g.indexOf("a/"), l = f.parentNode.offsetWidth
                } catch (n) {
